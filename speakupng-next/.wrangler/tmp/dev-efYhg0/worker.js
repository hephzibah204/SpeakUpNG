...
var worker_default = {
  async fetch(request, env3) {
    ...
    if (url.pathname.startsWith("/api/")) {
      return handleApiRequest(request, env3);
    }
    return env3.DB.prepare(
      "SELECT * FROM static_files WHERE path = ?"
    ).bind(url.pathname).first().then((file) => {
      if (!file) {
  ...
  async scheduled(event, env3, ctx) {
    if (event.scheduled.type === "news_ingestion") {
      await runNewsIngestion(env3);
    }
  }
};
async function handleApiRequest(request, env3) {
  const url = new URL(request.url);
  ...
  try {
    switch (path) {
      case "officials":
        return await handleOfficialsApi(request, env3);
      case "politicians":
        return await handlePoliticiansApi(request, env3);
      case "ratings":
        return await handleRatingsApi(request, env3);
      case "news":
        return await handleNewsApi(request, env3);
      case "search":
        return await handleSearchApi(request, env3);
      case "admin/secrets":
        return await handleAdminSecretsApi(request, env3);
      default:
        return new Response("Not found", { status: 404 });
    }
  } catch (error3) {
...
async function handleOfficialsApi(request, env3) {
  const url = new URL(request.url);
  ...
  const results = await env3.DB.prepare(query).bind(...params).all();
  ...
  const countResult = await env3.DB.prepare(countQuery).bind(...countParams).first();
  return new Response(
    JSON.stringify({
...
__name(handleOfficialsApi, "handleOfficialsApi");
async function handlePoliticiansApi(request, env3) {
  const url = new URL(request.url);
  const search = url.searchParams.get("search") || "";
  const page = parseInt(url.searchParams.get("page") || "1");
...
__name(handlePoliticiansApi, "handlePoliticiansApi");
async function handleRatingsApi(request, env3) {
  const url = new URL(request.url);
...
__name(handleRatingsApi, "handleRatingsApi");
async function handleNewsApi(request, env3) {
  const url = new URL(request.url);
  const search = url.searchParams.get("search") || "";
  const page = parseInt(url.searchParams.get("page") || "1");
  const limit = parseInt(url.searchParams.get("limit") || "20");
...
async function handleSearchApi(request, env3) {
  const url = new URL(request.url);
  ...
  const officialsQuery = env3.DB.prepare(
  ...
  const politiciansQuery = env3.DB.prepare(
  ...
  const newsQuery = env3.DB.prepare(
...
__name(handleSearchApi, "handleSearchApi");
async function handleAdminSecretsApi(request, env3) {
...
__name(handleAdminSecretsApi, "handleAdminSecretsApi");
async function runNewsIngestion(env3) {
  console.log("Running news ingestion...");
}
__name(runNewsIngestion, "runNewsIngestion");
...
// .wrangler/tmp/bundle-f6ZJsE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-f6ZJsE/middleware-loader.entry.ts
