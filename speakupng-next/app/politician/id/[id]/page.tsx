...
export default function PoliticianIdRedirect() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/politicians/${encodeURIComponent(id)}`)
      .then(res => res.json())
      .then(data => {
        if (data.politician) {
          const p = data.politician;
          router.replace(`/politician/${slugify(p.full_name)}--${p.id}`);
        } else {
          setError('Politician not found');
        }
      })
      .catch(() => {
        setError('Error loading politician profile');
      });
  }, [id, router]);
